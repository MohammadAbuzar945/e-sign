...
export async function action({ request }: { request: Request }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: false, error: 'Invalid JSON body', detail: message }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  if (!event || typeof event !== 'object' || !event.event || !event.data) {
    ...
    return new Response(JSON.stringify({ success: false, error: 'Missing event or data in payload' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    });
  }
