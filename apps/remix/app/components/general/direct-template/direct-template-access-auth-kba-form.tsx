...
type TDirectTemplateAccessAuthKBAFormSchema = z.infer<typeof ZDirectTemplateAccessAuthKBAFormSchema>;

type DirectTemplateAccessAuthKBAFormProps = {
  token: string;
  onSubmit: () => void;
};
...
export const DirectTemplateAccessAuthKBAForm = ({
  ...
  const form = useForm<TDirectTemplateAccessAuthKBAFormSchema>({
    resolver: zodResolver(ZDirectTemplateAccessAuthKBAFormSchema),
    defaultValues: {
      answer: '',
    },
  });
  ...
  const onFormSubmit = async (values: TDirectTemplateAccessAuthKBAFormSchema) => {
  ...
  if (!data) {
    return (
      <Alert variant="destructive" padding="tight" className="text-sm">
        <Trans>Unable to load KBA challenge for this template.</Trans>
      </Alert>
    );
  }
