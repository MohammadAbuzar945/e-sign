...
/**
 * Skip rate limiting for IPs in NEXT_PRIVATE_API_RATE_LIMIT_EXEMPT_IPS.
 */
const withApiRateLimitExemptSkip = (middleware: MiddlewareHandler): MiddlewareHandler => {
  return async (c, next) => {
    if (shouldSkipApiRateLimit(c.req.raw)) {
      await next();
      return;
    }
 ...
 */
const apiV1RateLimitMiddleware = withApiRateLimitExemptSkip(createRateLimitMiddleware(apiV1RateLimit));
const apiV2RateLimitMiddleware = withApiRateLimitExemptSkip(createRateLimitMiddleware(apiV2RateLimit));
