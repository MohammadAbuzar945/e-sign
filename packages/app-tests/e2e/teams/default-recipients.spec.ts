...
test.describe('Default Recipients', () => {
  test('[DEFAULT_RECIPIENTS]: default recipients are added to documents created via UI', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').last().fill('regular-signer@nomiadocs.com');
    await page
    ...
    await expect(async () => {
      ...
      const regularSigner = envelope.recipients.find((r) => r.email === 'regular-signer@nomiadocs.com');
  ...
  test.skip('[DEFAULT_RECIPIENTS]: default recipients cannot be removed from a document', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').first().fill('regular-signer@nomiadocs.com');
    await page
      .getByPlaceholder(/Recipient/)
      .first()
      .fill('Regular Signer');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created via API have default recipients', async ({ request }) => {
    ...
    const payload: TCreateEnvelopePayload = {
      ...
      recipients: [
        {
          email: 'api-recipient@nomiadocs.com',
          name: 'API Recipient',
    ...
    const apiRecipient = envelope.recipients.find((r) => r.email === 'api-recipient@nomiadocs.com');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created from template have default recipients', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email').fill('template-recipient@nomiadocs.com');
    ...
    const templateRecipient = document.recipients.find((r) => r.email === 'template-recipient@nomiadocs.com');
