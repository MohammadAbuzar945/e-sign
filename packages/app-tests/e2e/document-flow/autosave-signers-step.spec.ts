...
const addSignerAndSave = async (page: Page) => {
  await page.getByPlaceholder('Email').fill('recipient1@nomiadocs.com');
...
test.describe('AutoSave Signers Step', () => {
  test('should autosave the signers addition', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients[0].email).toBe('recipient1@nomiadocs.com');
  ...
  test('should autosave the signer update', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email').fill('manager@nomiadocs.com');
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients.length).toBe(1);
      expect(retrievedRecipients[0].email).toBe('manager@nomiadocs.com');
  ...
  test('should autosave the signing order change', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').nth(1).fill('recipient2@nomiadocs.com');
    await page.getByLabel('Name').nth(1).fill('Recipient 2');

    await page.getByRole('button', { name: 'Add Signer' }).click();

    await page.getByTestId('signer-email-input').nth(2).fill('recipient3@nomiadocs.com');
    ...
    await expect(async () => {
      ...
      const firstRecipient = retrievedRecipients.find((r) => r.email === 'recipient1@nomiadocs.com');
      const secondRecipient = retrievedRecipients.find((r) => r.email === 'recipient2@nomiadocs.com');
      const thirdRecipient = retrievedRecipients.find((r) => r.email === 'recipient3@nomiadocs.com');
